...
PRIMARY_INPUT_CASES = {"float": 0.4980392156862745, "int": 127}
...
RGB_CASES = {"float": (0.5, 0.5, 0.5), "int": (128, 128, 128)}
...
GAMUT_CASES = {"inside": [0.333, 0.333], "outside": [0.001, 0.001]}
...
class TestStimulationDevice(unittest.TestCase):
    ...
    def test_assert_wavelengths_are_valid(self):
        self.device._assert_wavelengths_are_valid(CALIBRATION_WAVELENGTHS)

    def test_assert_resolutions_are_valid(self):
        self.device._assert_resolutions_are_valid(PRIMARY_RESOLUTIONS)
    ...
    def test_w2s(self):
        msg = (
            "Test cases should be exactly the same when converted with "
            + "s2w and w2s"
        )
