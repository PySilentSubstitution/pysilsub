...
class BinocularStimulationDevice:
    ...
    def objective_function(self, x0, settings):
        ...
        if self.anchor=='left':
            s1 = [self.left.predict_primary_spd(primary, setting).sum() 
                   for primary, setting in enumerate(settings)]
            s1 = np.array(s1)
    
            
            s2 = [self.right.predict_primary_spd(primary, setting).sum() 
...
if __name__=='__main__':
    ...
    for s, r in zip(settings, results):
        S1.predict_multiprimary_spd(r).plot()
        S2.predict_multiprimary_spd(s).plot()
        plt.ylim(0, 2.8e6)
        plt.show()