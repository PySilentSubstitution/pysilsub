...
bg_settings = np.array([0.5] * 10)
ss.background = bg_settings

contrast_mods = [
    ss.pseudo_inverse_contrast(bg_settings, [tc, 0, 0, 0])
    for tc in target_contrasts
]

plt.figure()
plt.plot(
    ss.predict_multiprimary_spd(bg_settings + contrast_mods[0]),
    lw=1,
    label="+ve",
)
plt.plot(ss.predict_multiprimary_spd(bg_settings), lw=1, label="background")
plt.plot(
    ss.predict_multiprimary_spd(bg_settings + contrast_mods[-1]),
    lw=1,
    label="-ve",
)
plt.legend()
...
palette = sns.diverging_palette(
    220, 20, n=len(contrast_mods), l=65, as_cmap=False
)
...
constraints = {"fun": ss.silencing_constraint, "type": "eq"}
